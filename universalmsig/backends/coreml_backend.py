...
class CoreMLBackend(BaseBackend):
    ...
    Produces:
      \x07 <model>_coreml_spec.json     \u0007F model spec for inspection / debugging
      \x07 <model>_mil_graph.py         \u0007F executable MIL graph Python script
      \x07 <model>.mlpackage            \u0007F compiled CoreML bundle (needs coremltools)
    """
    ...
    def validate(self, sig: ModelSignature) -> list[str]:
        ...
        # Note: layout information is informational and moved to compile() metadata
        return warnings
    ...
    def compile(
        ...
        **kwargs: Any,
        ...
        meta = {
            ...
            "layout_note":   "Transformer uses (batch, seq, hidden) — no NHWC transpose needed for NLP models",
        }
    ...
    def _build_mil_script(self, sig: ModelSignature) -> str:
        ...
        lines = [
            ...
            "",
            "GQA Fix: KV heads are broadcast to match Q heads before the attention",
            "matmul (CoreML MIL does not handle mismatched Q/KV head dims).",
            "The expansion is an interleaved repeat (HF repeat_kv semantics):",
            "each KV head is repeated GQA_RATIO times CONSECUTIVELY, so Q head h",
            "attends to KV head h // GQA_RATIO. A flat tile would give the order",
            "[k0,k1,k0,k1,...] and pair 12 of 14 Q heads with the wrong KV head.",
            "",
            ...
            "",
            "    # K,V projections — KV heads ({}) not {}".format(sig.num_kv_heads, sig.num_heads),
        ...
        if is_gqa:
            lines += [
                f"    # ── GQA UNROLLING ─────────────────────────────────────────────",
                f"    # KV heads must be broadcast {sig.num_kv_heads} → {sig.num_heads} before the score",
                f"    # matmul, with INTERLEAVED repeats (HF repeat_kv): each KV head",
                f"    # repeated {gqa_ratio}x consecutively, so Q head h reads KV head h//{gqa_ratio}.",
                f"    # NOTE: a flat mb.tile on the head axis would repeat the whole",
                f"    # block ([k0,k1,k0,k1,...]) and mispair Q and KV heads — expand",
                f"    # a new axis, tile it, then collapse:",
                f"    k = mb.reshape(x=k, shape=[1, {sig.num_kv_heads}, 1, -1, {head_dim}])",
                f"    k = mb.tile(x=k, reps=[1, 1, {gqa_ratio}, 1, 1])",
                f"    k = mb.reshape(x=k, shape=[1, {sig.num_heads}, -1, {head_dim}])  # (B, {sig.num_heads}, S, D)",
                f"    v = mb.reshape(x=v, shape=[1, {sig.num_kv_heads}, 1, -1, {head_dim}])",
                f"    v = mb.tile(x=v, reps=[1, 1, {gqa_ratio}, 1, 1])",
                f"    v = mb.reshape(x=v, shape=[1, {sig.num_heads}, -1, {head_dim}])  # (B, {sig.num_heads}, S, D)",
                f"    # ── END GQA UNROLLING ──────────────────────────────────────────",
                "",
            ]
        else:
